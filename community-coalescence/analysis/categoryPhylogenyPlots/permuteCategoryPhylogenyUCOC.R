...
# Permute UC with OC ASVs and calculate null distribution of closest phylogenetic distances.
UCOCASVDistancesPermuted <- foreach(x=comboPassages, .combine="rbind") %dopar% {
  currCombo <- sub("_.*","",x)
  currPassage <- sub(".*_","",x)
  ...
  if (numUCASVs >=1 & numOCASVs >= 1) {
    UCOCASVDistancesPermuted <- foreach(y=seq(1:1000), .combine="rbind") %dopar% {
      UCASVsDf <- currASVsDf %>% 
        slice_sample(n=numUCASVs)
      OCASVsDf <- currASVsDf %>%
        filter(!(OTU %in% UCASVsDf$OTU)) %>% 
...
write.table(UCOCASVDistancesPermuted, "UCOCASVDistancesPermuted.txt", quote=FALSE, row.names=FALSE, sep="\t")
