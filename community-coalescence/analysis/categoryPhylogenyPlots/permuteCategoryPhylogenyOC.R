...
# Analyze closest neighbors for overcolonizing ASVs. ----------------------

# Permute OC ASVs and calculate null distribution of closest phylogenetic distances.
OCASVDistancesPermuted <- foreach(x=comboPassages, .combine="rbind") %dopar% {
  currCombo <- sub("_.*","",x)
  currPassage <- sub(".*_","",x)
  currASVsDf <- theoretical_mixtures_df_categorized %>% 
    ...
    unique()
  numASVs <- n_distinct(currASVsDf %>% filter(category=="overcolonizing") %>% pull(OTU))
  if (numASVs>1) {
    OCASVDistancesPermuted <- foreach(y=seq(1:1000), .combine="rbind") %dopar% {
      OCASVsDf <- currASVsDf %>% 
      ...
      distanceDf <- foreach(z=allCurrOCASVs, .combine="rbind") %dopar% {
        ...
        currDf <- OCASVsDf %>% 
          filter(OTU!=z)
        currOCASVs <- unique(currDf$OTU)
        distanceDf <- distDf %>% 
          filter(OTU1==z & OTU2 %in% currOCASVs) %>% 
...
write.table(OCASVDistancesPermuted, "OCASVDistancesPermuted.txt", quote=FALSE, row.names=FALSE, sep="\t")