...
# Filtering based on summary stats for high abundance one parent ASVs --------

# parentDiffSign = case_when(parentDiff > 0 ~ "positive", 
#                            parentDiff == 0 ~ "flat",
#                            parentDiff < 0 ~ "negative")) %>% 
#   group_by(combo, passage, OTU, mixtureType) %>% 
#   mutate(
#     # Annotate cases in which two replicates travel in opposite directions.
#     parentDiffSign=ifelse("positive" %in% parentDiffSign & "negative" %in% parentDiffSign & max(abs(parentDiff)) >= 0.42, 
#                           "mixed", parentDiffSign),
#     # If only one replicate is present, mark as noisy or low abundance.
#     parentDiffSign=ifelse("flat" %in% parentDiffSign & max(abs(parentDiff)) >= 0.42,
#                           "noisy", parentDiffSign),
#     parentDiffSign=ifelse(!highAbundanceParentLeft & !highAbundanceParentRight,
#                           "lowAbundance", parentDiffSign)) %>% 
#   ungroup() %>% 
#   group_by(combo, passage, OTU) %>% 
#   mutate(
#          parentDiffSign = case_when("positive" %in% parentDiffSign ~ "positive",
#                                     "flat" %in% parentDiffSign ~ "flat",
#                                     "negative" %in% parentDiffSign ~ "negative",
#                                     "mixed" %in% parentDiffSign ~ "mixed",
#                                     "noisy" %in% parentDiffSign ~ "noisy",
#                                     "lowAbundance" %in% parentDiffSign ~ "lowAbundance")) %>% 
#   ungroup()
# 
# mutate(category=case_when(is.na(parentDiff) ~ "lowAbundance",
#                           parentDiff==0 ~ "noisy",
#                           parentDiff > 0 & parentDiff < 0.42 ~ "flat",
#                           parentDiff >= 0.42 ~ "other"))
# group_by(OTU, combo, passage) %>%
#   mutate(category = case_when("lowAbundance" %in% category ~ "lowAbundance",
#                               "noisy" %in% category ~ "noisy",
#                               "flat" %in% category ~ "flat",
#                               "other" %in% category ~ "other"))
...
# # Group ASVs into behavior categories.
# test <- theoretical_mixtures_df %>% 
#   left_join(ASV_sum, by=c("OTU","Family","OTUnum","combo","passage")) %>% 
#   mutate(absDoseDifference = ifelse(abs(a_doseDifference) < 0.1, 0.1, abs(a_doseDifference))) %>%
#   group_by(combo, passage, OTU) %>% 
#   mutate(category = case_when(
#     # ASV not present at high abundance in either parent or in mixture ratios.
#     !highAbundanceParentLeft & !highAbundanceParentRight & avgdifference < 0.42 ~ "lowAbundance",
#     # ASV not present at high abundance in either parent but present in mixture ratios.
#     !highAbundanceParentLeft & !highAbundanceParentRight & avgdifference >= 0.42 ~ "noisy",
#     # ASV shows mixed directions of change in different replicates.
#     0 %in% parentDiff & max(abs(parentDiff), na.rm=TRUE) >= 0.42 ~ "noisy",
#     min(parentDiff, na.rm=TRUE) < 0 & max(parentDiff, na.rm=TRUE) > 0 & max(abs(parentDiff), na.rm=TRUE) >= 0.42 ~ "noisy",
#     # ASV does not change by at least 0.42 between parents in at least one replicate.
#     min(abs(parentDiff), na.rm=TRUE) > 0 & max(abs(parentDiff), na.rm=TRUE) < 0.42 ~ "parent-flat",
#     # ASV shows high variation across mixture ratios without much overall change.
#     (highAbundanceParentLeft | highAbundanceParentRight) & 
#       (a_doseDistance / absDoseDifference > 4.5 | t_rangeAvg > 0.75) ~ "noisy",
#     # ASV does not change by at least 0.42 between parents in at least one replicate and is not noisy.
#     categoryParents=="parent-flat" & (a_doseDistance / absDoseDifference) <= 4.5 ~ "flat",
#     
#   ))

# Filter to similar theoretical replicates (exclude outliers).
# Exclude ASVs with high dose distance but not dose difference.
# Categorize ASVs based on summary stat cutoffs.
# theoretical_mixtures_HighAbundanceOneParent_summaryFiltered <- theoretical_mixtures_df %>% 
#   left_join(ASV_sum, by=c("OTU","Family", "OTUnum", "combo","passage")) %>% 
#   mutate(
#     # Add pseudocount of 10^-2 to prevent 0 values from interfering with calculation.
#     absDoseDifference = ifelse(abs(a_doseDifference) < 0.1, 0.1, abs(a_doseDifference)),
#     noisyRatio = a_doseDistance / absDoseDifference,
#     # Final categorizations.
#     category=case_when(
#       # Low abundance ASVs: theoretical 0 or very low in only 1 replicate.
#       category=="lowAbundance" & avgdifference < 0.42 ~ "lowAbundance",
#       # Noisy: high variation across mixture ratios without much overall change; high diff between replicates.
#       category=="lowAbundance" & avgdifference >= 0.42 ~ "noisy",
#       category=="noisy" ~ "noisy",
#       category %in% c("flat", "other") & noisyRatio > 4.5 | t_rangeAvg > 0.75 ~ "noisy",
#       # Flat: low change between replicates, both replicates same direction or close
#       category=="flat" & noisyRatio <= 4.5 ~ "flat",
#       # Undercolonizing.
#       category=="other" & noisyRatio <= 4.5 & t_rangeAvg <= 0.75 &
#         avgdifference <= -0.2 & abs(a_doseDifference) <= 0.155 ~
#         "undercolonizing",
#       # Overcolonizing.
#       category=="other" & noisyRatio <= 4.5 & t_rangeAvg <= 0.75 &
#         avgdifference >= 0.2 & abs(a_doseDifference) <= 0.155 ~ 
...
#       # Dose-dependent.
#       category=="other" & noisyRatio <= 4.5 & t_rangeAvg <= 0.75 &
#         (abs(avgdifference) < 0.2 | abs(a_doseDifference) > 0.155) ~ 
#         "dose-dependent"),
#     categorySum = case_when(
#       category=="lowAbundance" ~ "lowAbundance",
#       category=="noisy" ~ "noisy",
#       category=="flat" ~ "flat",
#       category=="undercolonizing" &
#         avgdifference <= -0.2 & abs(a_doseSumDifference) <= 0.3 ~ "undercolonizing",
#       category=="overcolonizing" &
#         avgdifference >= 0.2 & abs(a_doseSumDifference) <= 0.3 ~ "overcolonizing",
#       category=="dose-dependent" & 
#         (abs(avgdifference) < 0.2 & abs(a_doseSumDifference) > 1.5) | 
#         abs(a_doseSumDifference) > 0.3 ~ "dose-dependent",
#       category=="dose-dependent" & 
#         abs(avgdifference) < 0.2 & abs(a_doseSumDifference) < 1.5 ~ "actualFlat"
#     ))
...
# Create theoretical phyloseq filtered to 10^-4. --------------------------

# Generate OTU table with OTUs as rownames.
# df_otu_raw_theoretical_relAb4 <- theoretical_mixtures_df %>% 
#   group_by(passage, label, line) %>% 
#   mutate(count=ceiling(rel_abundance * 131515)) %>% 
#   ungroup() %>% 
#   filter(filterRelAb4==TRUE) %>% 
#   mutate(sample=paste(line, label, passage, sep="_")) %>% 
#   select(OTU, count, sample) %>% 
#   pivot_wider(names_from=sample, values_from=count) %>% 
#   ungroup()
# df_otu_raw_theoretical_relAb4[is.na(df_otu_raw_theoretical_relAb4)] <- 0
# otu_rownames_relAb4 <- df_otu_raw_theoretical_relAb4$OTU
# df_otu_raw_theoretical_relAb4 <- df_otu_raw_theoretical_relAb4 %>% select(!OTU)
# rownames(df_otu_raw_theoretical_relAb4) <- otu_rownames_relAb4
# 
# # Create theoretical phyloseq.
# ps_theoretical_relAb4 <- phyloseq(otu_table(as.matrix(df_otu_raw_theoretical_relAb4), taxa_are_rows=TRUE),
#                            tax_table(as.matrix(df_tax_raw)),
#                            sam_theoretical)
# 
# # Create theoretical phyloseq filtered to 10^-3. --------------------------
# 
# # Generate OTU table with OTUs as rownames.
# df_otu_raw_theoretical_relAb3 <- theoretical_mixtures_df %>% 
#   group_by(passage, label, line) %>% 
#   mutate(count=ceiling(rel_abundance * 131515)) %>% 
#   ungroup() %>% 
#   filter(filterRelAb3==TRUE) %>% 
#   mutate(sample=paste(line, label, passage, sep="_")) %>% 
#   select(OTU, count, sample) %>% 
#   pivot_wider(names_from=sample, values_from=count) %>% 
#   ungroup()
# df_otu_raw_theoretical_relAb3[is.na(df_otu_raw_theoretical_relAb3)] <- 0
# otu_rownames_relAb3 <- df_otu_raw_theoretical_relAb3$OTU
# df_otu_raw_theoretical_relAb3 <- df_otu_raw_theoretical_relAb3 %>% select(!OTU)
# rownames(df_otu_raw_theoretical_relAb3) <- otu_rownames_relAb3
# 
# # Create theoretical phyloseq.
# ps_theoretical_relAb3 <- phyloseq(otu_table(as.matrix(df_otu_raw_theoretical_relAb3), taxa_are_rows=TRUE),
#                                   tax_table(as.matrix(df_tax_raw)),
#                                   sam_theoretical)
